...
const DungeonGenerator_1 = require("./procgen/DungeonGenerator");
const ParticleEngine_1 = require("./graphics/ParticleEngine");
const AudioSynthesizer_1 = require("./audio/AudioSynthesizer");
const SkillTreeMatrix_1 = require("./gameplay/SkillTreeMatrix");
const QuestManager_1 = require("./gameplay/QuestManager");
const StateEngine_1 = require("./core/StateEngine");
const ClassSystem_1 = require("./gameplay/ClassSystem");
...
class AAAFullGameApp {
    canvas;
    ctx;
    stateEngine = StateEngine_1.GlobalStateEngine.getInstance();
    dungeonGen = new DungeonGenerator_1.DungeonGenerator(45, 45);
    dungeonData;
    selectedClass = 'Warrior';
    playerPos = new Math2D_1.Vector2D(200, 200);
    playerVel = new Math2D_1.Vector2D();
    playerHp = 140;
    playerMaxHp = 140;
    playerResource = 100;
    playerMaxResource = 100;
    gold = 150;
    level = 1;
    xp = 0;
    maxXp = 100;
    keys = new Set();
    particles = new ParticleEngine_1.ParticleEngine();
    audio = new AudioSynthesizer_1.AudioSynthesizer();
    skills = new SkillTreeMatrix_1.SkillTreeMatrix();
    quests = new QuestManager_1.QuestManager();
    powerups = [];
    enemies = [];
    floatingTexts = [];
    audioMuted = false;
    ...
    startDungeonGame() {
        const classDef = ClassSystem_1.ClassSystem.getClass(this.selectedClass);
    ...
    quickSaveGame() {
        SecuritySaveManager_1.SecuritySaveManager.saveGame('quick_save', {
            slotId: 'quick_save',
            timestamp: Date.now(),
    ...
    loadSavedGame() {
        const data = SecuritySaveManager_1.SecuritySaveManager.loadGame('quick_save');
        if (data) {
    ...
    toggleAudio() {
        ...
        if (btn)
            btn.innerText = this.audioMuted ? '🔇 Audio OFF' : '🔊 Audio ON';
    ...
    spawnEntitiesAndPowerups() {
        ...
        for (let i = 1; i < this.dungeonData.rooms.length; i++) {
            ...
            const center = new Math2D_1.Vector2D((rm.x + rm.width / 2) * 32, (rm.y + rm.height / 2) * 32);
            this.enemies.push({
                pos: center.clone(),
                hp: def.hp,
                maxHp: def.hp,
            ...
            this.powerups.push({
                pos: new Math2D_1.Vector2D(center.x + (Math.random() - 0.5) * 60, center.y + (Math.random() - 0.5) * 60),
                type: pType,
                color: pColor,
    ...
    setupInputs() {
        window.addEventListener('keydown', (e) => {
            ...
            if (e.key === '2')
                this.castHeroSkill(1);
            if (e.key === '3')
                this.castHeroSkill(2);
            if (e.key === '4')
                this.castHeroSkill(3);
            if (e.key === '5')
                this.castHeroSkill(4);
            if (e.key === ' ') {
            ...
            if (e.key === 'i' || e.key === 'I')
                this.toggleModal('screen-inventory');
            if (e.key === 'k' || e.key === 'K')
                this.toggleModal('screen-skill-tree');
            if (e.key === 'q' || e.key === 'Q')
                this.toggleModal('screen-quest-log');
            if (e.key === 'Escape')
                this.togglePause();
        });
    ...
    castHeroSkill(index) {
        if (this.stateEngine.getState() === 'Paused' || this.stateEngine.getState() === 'MainMenu')
            return;
    ...
    dealAreaDamage(radius, damageAmount, isSpecial) {
        this.enemies.forEach((enemy, idx) => {
            if (enemy.pos.distance(this.playerPos) < radius) {
                ...
                this.addFloatingText(isSpecial ? '💥 CRIT -' + damageAmount : '-' + damageAmount, enemy.pos, isSpecial ? '#fbbf24' : '#ef4444');
    ...
    render() {
        ...
        if (this.stateEngine.getState() === 'MainMenu')
            return;
        this.ctx.save();
    ...
    renderHUD(width, height) {
        ...
        this.ctx.fillStyle = '#38bdf8';
        this.ctx.fillRect(mmX + this.playerPos.x * scaleX - 3, mmY + this.playerPos.y * scaleY - 3, 6, 6);
